...
mearble_mean_utilities = []
for marble_colors in all_marble_colors:
    marble_colors = np.array(ast.literal_eval(marble_colors))
    stim_zeros = np.count_nonzero(marble_colors == 0)
    stim_ones = np.count_nonzero(marble_colors == 1)
    stim_twos = np.count_nonzero(marble_colors == 2)
    mean_utility = ((stim_zeros * 4) + (stim_ones * 3) + (stim_twos * 2)) / 9
    mearble_mean_utilities.append(mean_utility)
    ...
    if(not args.is_eval_only):
        for stimuli_set in [0,1,2,3,4,5]:
            ...
            if(not os.path.exists(exp_dir + "/set" + str(stimuli_set))):
                os.mkdir(exp_dir + "/set" + str(stimuli_set))
            ...
            utilities = np.array(mearble_mean_utilities)
